...
from datasets.coordinate_transformation import convert_3dbox_to_8corner
from datasets.datafusion import datafusion2Dand3D
from detection.detection import Detection_3D_Fusion, Detection_3D_only
...
class DeepFusion(object):
    ...
    def update(self, detection_3D_fusion, detection_3D_only, additional_info, calib_file):
        ...
        self.tracker.update(detection_3D_fusion, detection_3D_only, calib_file, iou_threshold=0.5)
        ...
        return outputs  # (id,h,w,l,x,y,z,theta,alpha,type,x1,y1,x2,y2,score)
...
if __name__ == '__main__':
    ...
    detections_name_3D = '3d_pr'
    detections_name_2D = '2d_rrc'
    ...
    save_root = 'results/pr_rrc/xxxx'  # The root directory where the result is saved
    ...
    tracker = DeepFusion(max_age=25, min_hits=2)  # Tracker initialization
    ...
    for seq_file_3D, image_filename in zip(detection_file_list_3D, image_files):
        ...
        seq_dets_3D[:, 6] = (seq_dets_3D[:, 6]-min(seq_dets_3D[:, 6]))/(max(seq_dets_3D[:, 6])-min(seq_dets_3D[:, 6]))
        ...
        for frame, img0_path in zip(range(min_frame, max_frame + 1), image_filenames):
            ...
            detection_3D_fusion, detection_3D_only = datafusion2Dand3D(dets_3D_camera, dets_2D, dets_3Dto2D_image, additional_info)
            """
            ...
            if not detection_2D_only_tlwh.shape == (0,):
                _ = 0
            
            for i in range(len(detection_3D_fusion['dets_3d_fusion'])):
                detection_3D_fusion['dets_3d_fusion'][i][6]=detection_3D_fusion['dets_3d_fusion'][i][6]/np.pi*180
            """
            start_time = time.time()
            trackers = tracker.update(detection_3D_fusion, detection_3D_only, additional_info, calib_file_seq)
