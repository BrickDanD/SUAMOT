...
def datafusion2Dand3D(detections_3D_camera, detection_2D, detection_3Dto2D, additional_info):
    ...
    if min(iou_matrix.shape) > 0:
        a = (iou_matrix > iou_threshold).astype(np.int32)  # 判断iou大于阈值的值
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:  # 判断是否每个检测框最多只有一个检测框与之对应
            matched_indices = np.stack(np.where(a), axis=1)  # np.where(a)输出非零元素坐标，np.stack类似于转置
        else:
    ...
    for d, det in enumerate(detection_2D):  # 找出不匹配的二维检测框
        if d not in matched_indices[:, 0]:
            unmatched_detections_2D.append(d)

    for t, trk in enumerate(detection_3Dto2D):  # 找出不匹配的三维检测框
        if t not in matched_indices[:, 1]:
            unmatched_detection_3Dto2D.append(t)
            # unmatched_3Dto2D_additional_info.append(additional_info[t])

    for m in matched_indices:  # 找出匹配的检测框
        if iou_matrix[m[0], m[1]] < iou_threshold:  # 找出iou低于阈值的检测框
    ...
    """
    detection_3D_fusion, detection_3D_fusion_info = [], []
    for i in range(len(detections_3D_camera)):
        detection_3D_fusion.append(detections_3D_camera[i].tolist())
        detection_3D_fusion_info.append(additional_info[i].tolist())
    """
    ...
    return detection_3D_fusion, detection_3D_only
