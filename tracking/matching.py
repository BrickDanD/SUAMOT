...
import numpy as np
import time
from tracking.cost_function import iou3d, convert_3dbox_to_8corner, iou_batch
...
def associate_detections_to_trackers_fusion(detections, trackers, iou_threshold):
    ...
    detections = copy.copy(_detections)
    for i in range(len(detections)):
        detections[i].bbox[3] = detections[i].bbox[3] / 180 * np.pi
    """
...
def _associate_detections_to_trackers_fusion(detections, trackers, pose, iou_threshold):
    """
    Assigns detections to tracked object (both represented as bounding boxes)
    detections:  N x 8 x 3
    trackers:    M x 8 x 3
    ...
    for i in range(len(trackers)):
        trackers[i].pose[3] = trackers[i].pose[3] / 180 * np.pi
    for i in range(len(detections)):
        detections[i].bbox[3] = detections[i].bbox[3] / 180 * np.pi
    """
    ...
    for d in range(len(dets_8corner)):
        for t in range(len(trks_8corner)):
            a = detections[d].bbox[3]
            b = trackers[t].pose[3]

            if abs(a - b) > np.pi * 3 / 2:
                if a > 0:
                    b += np.pi * 2
                else:
                    b -= np.pi * 2
...
# def associate(tracks,detections,images):
